...
def alterar_registro(tree, atualizar_grid_callback=None):
    ...
    codigo, descricao, seg, perc = valores
    ...
    entry_percentual.insert(0, perc)
    ...
    def salvar():
        ...
        novo_perc = entry_percentual.get().strip()
        novo_seg = combo_segmento.get().strip()

        try:
            perc_float = float(novo_perc)
            if perc_float < 0:
                raise ValueError
        except ValueError:
